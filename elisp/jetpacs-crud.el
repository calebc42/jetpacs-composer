...
;; The engine half of this runtime — the vulpea/org-ql probes, `:ID:'
;; adoption, index-backed reads, the FILTER router, and the tables+checklist
;; extractor — lives in jetpacs-crud-vulpea.el.  Neither file requires the
;; other from here; the bundle inlines both and jetpacs-crud-orgapp.el
;; requires both.  These stubs are the byte-compile contract.
(declare-function jetpacs-crud--vulpea-p "jetpacs-crud-vulpea" ())
(declare-function jetpacs-crud-vulpea-ensure-source "jetpacs-crud-vulpea" (spec view))
(declare-function jetpacs-crud--view-notes-records "jetpacs-crud-vulpea" (spec view))
(declare-function jetpacs-crud--scan-notes-impl "jetpacs-crud-vulpea" (spec view))
(declare-function jetpacs-crud--reindex "jetpacs-crud-vulpea" (file))
(declare-function jetpacs-crud-vulpea-tables "jetpacs-crud-vulpea" (file heading))
(declare-function jetpacs-crud-vulpea-checklist "jetpacs-crud-vulpea" (file heading))

;; The note actions and tree scan below also touch vulpea directly (always
;; behind the `jetpacs-crud--vulpea-p' gate); stubs for the byte-compiler.
...
(defun jetpacs-crud--slug (text)
  "A filesystem-safe slug for TEXT: lowercased, trimmed, runs of
non-alphanumerics collapsed to dashes; \"note\" when TEXT holds none.
The shared basis for tabulated-group destination names and note file
names.  (This file once carried two shadowing definitions of this
function; this is the one that was live.)"
...
(defun jetpacs-crud--entry-prose (file &optional pos id)
  "Body prose for the heading/file entry addressed by POS or ID in FILE."
  (jetpacs-crud--with-source file
    (cond
     (id (jetpacs-crud--note-goto id))
     (pos (goto-char pos))
     (t (goto-char (point-min))))
    (let ((end (if (org-at-heading-p) (org-entry-end-position) (point-max))))
...
Opens a dedicated typed detail composition inside a full bottom sheet."
  (let* ((app (alist-get 'app args))
    ...
    (if (eq (plist-get view :kind) 'notes)
        (pcase-let* ((`(,_spec ,_view ,note) (jetpacs-crud--note-resolve args))
                     (file (vulpea-note-path note))
                     (record (cl-find id (jetpacs-crud--scan-notes spec view)
